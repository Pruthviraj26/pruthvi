                        ...
                        <tbody>
                          ...
                          <?php /*	 
                            ...
                            <td class="last">
															...
															<?php  }else{ ?>
																<button id="<?= $card->id ?>" value="APPROVED" class="card_status btn btn-default" title="View" data-toggle="tooltip"><i class="fa fa-eye"></i></button>
															<?php } ?>
															
															<?php if($card->status=="ACCEPTED") { ?>
																<a href="<?= ADMIN_URL.'/cards/changestatus/completed/'.$card->id ?>" class="btn btn-success" title="Complete" data-toggle="tooltip"><i class="fa fa-thumbs-up"></i></a>
															<?php } ?>
...
echo $this->element('form_start',['formTitle'=>$viewTitle,'form_id'=>'appprovalFrom']); ?>
	...
	$(document).ready(function() {
		...
		$(".card_status").click(function(){
			...
			if($value=='APPROVED'){
				...
				$.get("<?=ADMIN_URL ?>/cards/get/"+$id,function(response){
					response = JSON.parse(response)
					if(response.status=='SUCCESS'){
						card = response.data.card;
					
						$("#cardToAddBalane").val(card.id);
						$("#appprovalFrom h2").html('Wallet Address : ' + card.wallet_address +' <br> Card Number :  '+card.card_no+'' );	
						
						$("#balance_from_user").val(card.balance_from_user);
						$("#currency_title").html(card.currency.title +' Offer');				
						$("#product_offer_pecentage").val(card.product.offer_percentage);				
						$("#balance_from_admin").val(card.balance_from_admin);
						$("#balance_used").val(card.balance_used);
						$("#offer").val(card.offer);
						$("#status").val($value);
						
							$("#myModalLabel").html('Request for '+ card.product.title + ' By ' + card.user.first_name + ' ' + card.user.last_name);
						
						
						
						if(card.status!='PENDING'){
								
									$("#appprovalFrom h2").html('Detail of Card '+card.card_no + ' | '+  card.wallet_address );		
