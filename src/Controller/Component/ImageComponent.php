...
class ImageComponent extends Component{
    ...
    /*public function  initialize($config = array()){
				if($config){
					$this->$image_size = $config['image_size'];
				}else{
					$this->image_size = [];
	...
	public function save($file, $quality = 90) {
		$info = pathinfo($file);
            ...
            $file=$info['dirname'].DS.$info['filename'].'.'.$extension;
    $result = false;
		if (is_resource($this->image)) {
			...
			return ($result and imagedestroy($this->image));
		}else{
            return false;
        }
	...
	public function resize($width = 0, $height = 0, $default = 'd') {
		...
		if ($default == 'w') {
			...
			$scale = $this->info['width']<$this->info['height']?$scale_w:$scale_h;
		}
		 else {
